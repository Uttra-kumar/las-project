        ...
        <form id="salaryForm" onsubmit="saveSalary(event)">
            ...
            <div class="form-grid">
                <div class="form-group">
                    <label><i class="fas fa-user-tie"></i> Staff <span class="required">*</span></label>
                    <select name="staff_id" id="staff_id" required onchange="getStaffData()">
                        <option value="">-- Select --</option>
                        @foreach($staff as $member)
                        <option value="{{ $member->id }}">{{ $member->full_name }} ({{ $member->emp_id }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="month" id="month" required onchange="getStaffData()">
                        <option value="">-- Select --</option>
                        @foreach($months as $month)
                        <option value="{{ $month }}" {{ $month == date('F Y') ? 'selected' : '' }}>{{ $month }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="info-box" id="infoBox">
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <div class="info-value blue" id="staffName">-</div>
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value purple" id="staffDept">-</div>
                    </div>
                    <div class="info-item">
...
function getStaffData() {
    ...
    document.getElementById('staffName').textContent = 'Loading...';
    
    fetch(`{{ route('finance.staff-salary.get-staff-data') }}?staff_id=${staffId}&month=${month}`)
        .then(response => response.json())
        .then(response => {
            if (response.success) {
                const d = response.data;
                document.getElementById('infoBox').classList.add('active');
                document.getElementById('staffName').textContent = d.staff_name;
                document.getElementById('salaryAmount').textContent = '₹' + d.salary;
                document.getElementById('staffDept').textContent = d.department.charAt(0).toUpperCase() + d.department.slice(1);
...
function saveSalary(event) {
    ...
    if (!data.staff_id || !data.month || !data.payment_date) {
    ...
    fetch('{{ route("finance.staff-salary.store") }}', {
        method: 'POST',
        headers: {
